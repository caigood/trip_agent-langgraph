        ...
        <div class="top-info-section">
          ...
          <div class="left-info">
            ...
            <a-card id="budget" v-if="tripPlan.budget" title="💰 预算明细" :bordered="false" class="budget-card">
              ...
              <div v-if="tripPlan.budget.breakdown" class="budget-breakdown">
                <div class="breakdown-label">📝 预算说明:</div>
                <div class="breakdown-content">{{ tripPlan.budget.breakdown }}</div>
              </div>
            </a-card>
          </div>
          ...
          <a-collapse v-model:activeKey="activeDays" accordion>
            ...
            >
              ...
              <div class="day-info">
                ...
                <!-- 详细行程: 上午/下午/晚上 -->
                <div v-if="day.morning" class="info-row detail-row">
                  <span class="label">🌅 上午:</span>
                  <span class="value">{{ day.morning }}</span>
                </div>
                <div v-if="day.afternoon" class="info-row detail-row">
                  <span class="label">☀️ 下午:</span>
                  <span class="value">{{ day.afternoon }}</span>
                </div>
                <div v-if="day.evening" class="info-row detail-row">
                  <span class="label">🌙 晚上:</span>
                  <span class="value">{{ day.evening }}</span>
                </div>
...
const loadAttractionPhotos = async () => {
  ...
  tripPlan.value.days.forEach(day => {
    day.attractions.forEach(attraction => {
      const promise = fetch(`http://localhost:8001/api/poi/photo?name=${encodeURIComponent(attraction.name)}`)
        .then(res => res.json())
        .then(data => {
...
const exportAsImage = async () => {
  try {
    ...
    const canvas = await (html2canvas as any)(exportContainer, imageOptions)
...
const exportAsPDF = async () => {
  try {
    ...
    const canvas = await (html2canvas as any)(exportContainer, pdfOptions)
...
const initMap = async () => {
  try {
    ...
    let centerSet = false
    if (tripPlan.value && tripPlan.value.days) {
      for (const day of tripPlan.value.days) {
        if (day.attractions && day.attractions.length > 0) {
          const firstAttr = day.attractions[0]
          const normalizedLocation = normalizeLocation(firstAttr.location)
          if (normalizedLocation) {
            map.setCenter([normalizedLocation.longitude, normalizedLocation.latitude])
            centerSet = true
            break
          }
        }
    ...
    if (!centerSet && tripPlan.value && tripPlan.value.city) {
      const placeSearch = new AMap.PlaceSearch({
        city: tripPlan.value.city
      })
      placeSearch.search(tripPlan.value.city, (status: string, result: any) => {
...
const normalizeLocation = (location: any): { longitude: number; latitude: number } | null => {
  ...
  if (typeof location === 'object') {
    ...
    if (typeof location.longitude === 'string' && typeof location.latitude === 'string') {
...
const addAttractionMarkers = (AMap: any) => {
  ...
  tripPlan.value.days.forEach((day, dayIndex) => {
    day.attractions.forEach((attraction, attrIndex) => {
      const normalizedLocation = normalizeLocation(attraction.location)
      if (normalizedLocation) {
        allAttractions.push({
          ...attraction,
